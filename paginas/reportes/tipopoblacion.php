<?php
	...
	$sql = 
		"
			SELECT 
				poblacionclave AS poblacion, COUNT(correlativo)AS total 
			FROM 
				generales 
			WHERE 
				fecharegistro BETWEEN '$inicio' AND '$fin'
			GROUP BY 
				poblacionclave 
			ORDER BY 
				poblacionclave ASC;
	...
	if($filas > 0)
		{
			...
			while($fila = mysqli_fetch_assoc($result))
				{
					$tabla = $tabla . "<tr><td>" . $fila['poblacion'] . "</td><td style='text-align: right;'>" . $fila['total'] . "</td></tr>";
					$valoresx[$i] = $fila['poblacion'];
...
<html>
	<head>
		...
		<script type="text/javascript">
			...
			var layout = 
				{
					title: 'REGISTRO DE USUARIOS POR TIPO DE POBLACION', 
