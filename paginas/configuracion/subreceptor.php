...
?>
    <script type="text/javascript">
      function confirmar(){
        if(confirm('Esta seguro que desea de guardar')){
          document.ControlUsuarios.submit();
        }
      }
    </script>
		<form name="ControlUsuarios" method="POST" action="javascript: GuardarElementos('8');">
			<table class="table table-condensed table-striped table-responsive table-bordered" style="width: 90%; margin: 0 auto;">
				<thead>
					<tr>
						<th colspan="2">ADMINISTRACION DE SUBRECEPTOR</th>
					</tr>
				</thead>
				<tbody>
					<tr>
		...
		<table class="table table-bordered table-responsive table-striped table-condensed" style="width: 50%; margin: 0 auto;">
          ...
          <th>CODIGO</th>
			...
			<tbody>
				<?php
					$sql = "SELECT * FROM subreceptor ORDER BY correlativo ASC";
							...
							while($fila = mysqli_fetch_assoc($result))
								{
									echo
										"
											<tr>
												<td style='width: 10%;'>" . $fila['correlativo'] . "</td>
												<td style='width: 25%;'>" . $fila['codigo'] . "</td>
                        <td style='width: 75%;'>" . $fila['subreceptor'] . "</td>
											</tr>
										";
								}
						}
